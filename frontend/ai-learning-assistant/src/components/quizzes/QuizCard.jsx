...
const QuizCard = ({ quiz, onDelete }) => {
    return (
        <div className="group relative bg-white/80 backdrop-blur-xl border-2 border-slate-200 hover:border-fuchsia-300 rounded-2xl p-4 transition-all duration-200 hover:shadow-lg hover:shadow-fuchsia-500/10 flex flex-col justify-between">
            <button
                onClick={(e) => {
                    e.stopPropagation();
                    onDelete(quiz);
            ...
            <div className="space-y-4">
                {/* Score Badge */}
                <div className="inline-flex items-center gap-1.5 py-1 rounded-lg text-xs font-semibold">
                    <div className="flex items-center gap-1.5 bg-fuchsia-50 border border-fuchsia-200 rounded-lg px-3 py-1">
                        <Award
            ...
            {/* Action Button */}
            <div className="mt-2 pt-4 border-t border-slate-100">
                {(quiz?.userAnswers?.length || 0) > 0 ? (
                    <Link to={`/quizzes/${quiz?._id}/results`}>
                        <button className="group/btn w-full inline-flex items-center justify-center gap-2 h-11 bg-fuchsia-50 hover:bg-fuchsia-100 text-fuchsia-700 border border-fuchsia-200 font-semibold text-sm rounded-xl transition-all duration-200 active:scale-95 cursor-pointer">
                            <BarChart2 className="w-4 h-4" strokeWidth={2.5} />
                            View Results
                        </button>
                    </Link>
                    ...
                    <Link to={`/quizzes/${quiz?._id}`}>
                        <button className="group/btn relative w-full h-11 bg-gradient-to-r from-fuchsia-500 to-purple-500 hover:from-fuchsia-600 hover:to-purple-600 text-white font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-fuchsia-500/25 active:scale-95 overflow-hidden">
                            <span className="relative z-10 flex items-center justify-center gap-2">
                                <Play className="w-4 h-4" strokeWidth={2.5} />
                                Start Quiz
                            </span>
