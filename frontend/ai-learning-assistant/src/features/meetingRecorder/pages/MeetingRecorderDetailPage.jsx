...
// ─────────────────────────────────────────────────────────────────────────────
// AI Meeting Recorder — Meeting detail page
// Ported verbatim from Deploy-Whisper's MeetingDetail.jsx with two adjustments:
//   • wrapped in `.meeting-recorder-root` so the ported CSS stays scoped
//   • added a "← Back to recorder" link that points at MEETING_RECORDER_BASE
//     since the page is mounted at /meeting-recorder/:id (not /meeting/:id)
// All business logic — transcripts, summaries, language switching, video
// streaming, generate / regenerate — is unchanged.
// ─────────────────────────────────────────────────────────────────────────────

const SERVER_URL = API_URL;

function MeetingRecorderDetailPage() {
    const { id } = useParams();
    ...
    useEffect(() => {
        axios
            .get(`${SERVER_URL}/api/recordings/${id}`)
    ...
    useEffect(() => {
        axios
            .get(`${SERVER_URL}/api/transcripts/${id}`)
            .then((r) => {
    ...
    // Fetch summary (poll while it's still being generated)
    ...
    // language -> data (fallback to legacy top-level fields for old docs)
    ...
    const handleGenerate = async (lang) => {
        ...
        try {
            const res = await axios.post(
                `${SERVER_URL}/api/summaries/${id}/generate`,
                { language: lang }
            );
            setSummary(res.data);
            setLanguage(lang);
    ...
    return (
        <div className="meeting-recorder-root">
            <div className="mr-feature-header">
                <Link to={MEETING_RECORDER_BASE} className="mr-back-link">
                    <ChevronLeft size={14} /> Back to recorder
                </Link>
            </div>
                ...
                <div className="md-body">
                    <aside className="md-sidebar">
                        {tabs.map((tab) => (
                            ...
                            >
                                <span className="md-tab-emoji">
                                    {tab.emoji}
                                </span>
                                <span className="md-tab-label">
                                    {tab.label}
                                </span>
                            </button>
                        ))}
                            ...
                            <div className="md-panel">
                                ...
                                {!currentData ? (
                                    <p className="md-placeholder">
                                        ...
                                        summary first to see key points.
                                    </p>
...
export default MeetingRecorderDetailPage;
