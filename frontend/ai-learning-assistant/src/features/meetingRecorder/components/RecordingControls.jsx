...
// ─────────────────────────────────────────────────────────────────────────────
// AI Meeting Recorder — RecordingControls
// Ported verbatim from SayanMazumder75/Deploy-Whisper (client/src/components
// /RecordingControls.jsx). Only the import paths and the API_URL source have
// been adjusted for Vite + the new feature folder location. Behaviour is
// unchanged.
// ─────────────────────────────────────────────────────────────────────────────
...
function openFloatingStopWindow() {
    ...
    if (!popup) {
        alert(
            `Please allow popups for ${window.location.origin} — click the popup blocked icon in your address bar!`
        );
        return null;
    }
...
function RecordingControls({ onRecordingChange }) {
    ...
    const startRecording = async () => {
        ...
        try {
            ...
            const mediaRecorder = new MediaRecorder(
                ...
                {
                    mimeType:
                        videoTracks.length > 0 ? mimeType : "audio/webm",
                }
            );
            ...
            if (onRecordingChange)
                onRecordingChange(true, recordingIdRef.current);
    ...
    const stopRecording = async () => {
        ...
        if (chunkBufferRef.current.length > 0 && id) {
            const fullBlob = new Blob(chunkBufferRef.current, {
                type: mimeTypeRef.current,
            });
            console.log(
                `Uploading: ${(fullBlob.size / 1024 / 1024).toFixed(2)} MB`
            );
            ...
            try {
                await axios.post(
                    `${SERVER_URL}/api/recordings/${id}/upload`,
                    formData,
                    {
            ...
            try {
                await axios.post(
                    `${SERVER_URL}/api/recordings/${id}/chunk`,
                    audioFormData,
                    {
                        headers: { "Content-Type": "multipart/form-data" },
                    }
        ...
        if (id) {
            try {
                await axios.put(
                    `${SERVER_URL}/api/recordings/${id}/status`,
                    { status: "completed" }
                );
    ...
    return (
        <div className="recording-controls">
            {status === "error" && (
                <p className="recording-error">{errorMsg}</p>
            )}
                        ...
                        {SOURCE_OPTIONS.map((opt) => (
                            <button
                                ...
                                className={`source-option ${
                                    source === opt.value ? "active" : ""
                                }`}
                                onClick={() => setSource(opt.value)}
                                disabled={status === "stopping"}
                            >
                                <span className="source-opt-emoji">
                                    {opt.emoji}
                                </span>
                                <span className="source-opt-label">
                                    {opt.label}
                                </span>
                            </button>
                        ))}
            ...
            {status === "requesting" && (
                <p className="recording-hint">
                    Allow mic access, then select the tab to record.
                </p>
            )}
            {isRecording && (
                <p className="recording-hint">
                    🪟 Use the floating <strong>Stop</strong> window to stop
                    from any tab!
                </p>
            )}
            {!isRecording && status === "idle" && (
                <p className="recording-hint">
                    Click <strong>Start Recording</strong> — a floating stop
                    button will appear so you can stop from any tab.
                </p>
            )}
            {status === "stopping" && (
                <p className="recording-hint">
